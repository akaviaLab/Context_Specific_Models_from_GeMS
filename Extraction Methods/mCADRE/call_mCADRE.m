function tissueModel = call_mCADRE(model, gene_names, gene_exp, GPRmat, GPRrxns, manualCore, ht, checkv, eta, tol)
    %Run mCADRE with specified parameters
    %This script is an adapted version of the implementation from https://github.com/jaeddy/mcadre
    %Changes are indicated by "FIX" as comment.
    %Note that "Biomass_reaction" reaction is hard-coded to be active
    %
    %Input:
    %  model - consistent COBRA model struct (input model)
    %  gene_names - gene identiefiers corresponding to gene_exp ("findUsedGeneLevels.m")
    %  gene_exp - gene expression corresponding to gene_id ("findUsedGeneLevels.m")
    %  GPRmat - GPR matrix as returned by "extractGPRS.m" (parsedGPR)
    %  GPRrxns - reaction cell as returned by "extractGPRS.m" (corrRxn)
    %  manualCore - cell with reactions names that are manually added to
    %               the core reaction set (i.e. {'Biomass_reaction'})
    %  ht - reactions with expression higher than this threshold will be in 
    %       the core reaction set (expression threshold)
    %  checkv - Boolean variable:
    %             0: don't use salvage pathway and functionality check
    %             1: include salvage pathway and functionality check
    %  eta - tradeoff between removing core and zero-expression reactions (1/3)
    ...
    %Gene expression data [0,1], scaled w.r.t. upper threshold (ht)
    expression = gene_exp/ht;
...
function [PM, GM, C, NC, Z, model_C, pruneTime, cRes] = mcadre(model, G, U, confidenceScores, salvageCheck, metaboliteCheck, manualCore, method, GPRrxns, GPRmat, eta, tol)
...
%         end
        ...
        [GM, C, NC, P, Z, model_C] = rank_reactions(model, G, U, confidenceScores, manualCore, method, GPRrxns, GPRmat);
...
function [GM, C, NC, P, Z, model_C] = rank_reactions(model, G, U, confidenceScores, manualCore, method, ~, ~)
    ...
    if nargin > 4
        ...
        for i = 1:length(manualCore)
            ind = find(ismember(model.rxns,manualCore{i}));
            is_C_H(ind) = true;
    ...
    % therefore core is only expression evidence of 1. (core = expression > ht)
    C = model.rxns(E_X >= 1);
    model_C = C;
