function tissueModel = call_GIMME(model, expressionCol, threshold, obj_frac, tol)
    %Run GIMME with specified parameters
    %Input:
    ...
    [reactionActivity,reactionActivityIrrev,model2gimme,gimmeSolution] = solveGimme(model, objectiveCol, expressionCol, threshold);
    ...
    for i=1:nIrrevRxns
        if (expressionColIrrev(i,1) > -1)   %if not absent reaction
            if (expressionColIrrev(i,1) < cutoff)
                model2gimme.c(i,1) = cutoff-expressionColIrrev(i,1); %FIX: use expression level as weight
            end
        end
    end
