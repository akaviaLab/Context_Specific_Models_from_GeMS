function tissueModel = call_iMAT(model, manualRH, eps_param, expressionCol, lower_threshold, upper_threshold, tol, logfile, runtime)
    %Run iMAT with specified parameters
    %Input:
    %  model - consistent COBRA model struct (input model)
    %  manualRH - cell with reaction names (strings) that are manually put in
    %             the high confidence set
    %  eps_param - minimum flux threshold for "expressed" reactions (epsilon)
    %  expressionCol - reaction expression level. Must be same length as model.rxns
    %  lower_threshold - reactions with expression below this value are "non-expressed"
    %  upper_threshold - reactions with expression above this value are "expressed"
    ...
    %  runtime - maximum solve time for the MILP

    % Implementation adapted from the cobra toolbox (createTissueSpecificModel.m)    
    RHindex = find(expressionCol >= upper_threshold);
    RLindex = find(expressionCol >= 0 & expressionCol < lower_threshold);
    
    %Manually add defined core reactions to the core
    for i = 1:length(manualRH)
        rloc = find(ismember(model.rxns, manualRH{i}));
        if ~isempty(rloc) && isempty(intersect(RHindex,rloc))
            RHindex(end+1) = rloc; %#ok<AGROW>
        end
        if isempty(rloc)
            disp(['Manual added core reaction: ', manualRH{i}, ' not found'])
        end
    end
...
function solution = solveCobraMILP_loc(MILPproblem,varargin)
    ...
    %% glpk
     ...
     case 'gurobi5'
            ...
            %Sjoerd: write log file for gurobi
